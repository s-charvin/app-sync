...
func SystemData(ctx context.Context, pool *pgxpool.Pool, userID string, logger *slog.Logger) error {
	...
	for i, q := range entityInserts {
		if _, err := tx.Exec(ctx, q, userID); err != nil {
			return fmt.Errorf("seed entity %d: %w", i+1, err)
		}
	}

	// Binding tables — no user_id, filter templates by _sync_scope_id = sentinel
	bindingInserts := []string{
	...
	for i, q := range bindingInserts {
		if _, err := tx.Exec(ctx, q, userID, sentinelScope); err != nil {
			return fmt.Errorf("seed binding %d: %w", i+1, err)
		}
	}
