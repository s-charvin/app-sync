...
func (s *SyncService) installRegisteredTableCaptureTriggers(ctx context.Context) error {
	...
	return pgx.BeginFunc(ctx, s.pool, func(tx pgx.Tx) error {
		...
		for _, table := range s.config.RegisteredTables {
			...
			// Drop existing triggers gracefully. DROP TRIGGER requires table ownership
			// even with IF EXISTS; use a DO block to catch permission errors so that
			// non-owner roles can still install or reinstall triggers.
			dropStmts := []string{
				fmt.Sprintf(`DROP TRIGGER IF EXISTS %s ON %s`, registeredTableCaptureTriggerName, tableIdent),
				fmt.Sprintf(`DROP TRIGGER IF EXISTS %s ON %s`, registeredTableOwnerGuardTrigger, tableIdent),
			}
			for _, dropStmt := range dropStmts {
				if _, err := tx.Exec(ctx, fmt.Sprintf(`DO $$ BEGIN %s; EXCEPTION WHEN OTHERS THEN END; $$;`, dropStmt)); err != nil {
					return fmt.Errorf("drop trigger on %s: %w", table.normalizedKey(), err)
				}
			}

			// Create triggers, ignoring "already exists" errors that occur when
			// we couldn't drop the old trigger due to table ownership restrictions.
			createStmts := []string{
				fmt.Sprintf(`CREATE TRIGGER %s BEFORE INSERT OR UPDATE OR DELETE ON %s FOR EACH ROW EXECUTE FUNCTION sync.enforce_registered_row_owner()`,
					registeredTableOwnerGuardTrigger, tableIdent),
				fmt.Sprintf(`CREATE TRIGGER %s AFTER INSERT OR UPDATE OR DELETE ON %s FOR EACH ROW EXECUTE FUNCTION sync.capture_registered_row_change(%s, %s, %s)`,
					registeredTableCaptureTriggerName, tableIdent,
			...
			for _, createStmt := range createStmts {
				if _, err := tx.Exec(ctx, fmt.Sprintf(`DO $$ BEGIN %s; EXCEPTION WHEN duplicate_object THEN END; $$;`, createStmt)); err != nil {
					return fmt.Errorf("create trigger on %s: %w", table.normalizedKey(), err)
				}
			}
		}
